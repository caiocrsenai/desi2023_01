        ...
        <form method="POST" action="" id="stockForm" name="stockForm" novalidate>
            <label>
                ...
                <select name="id_product">
                    <option selected disabled style="display: none;" value="">Selecione seu produto</option>
                    <?php
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_object()) {
                            echo '<option value="' . $row->id . '" ' . ($stockInfos ? ($stockInfos->id_product == $row->id_product ? 'selected' : '') : '') . ' >' . $row->name . '</option>';
                        }
                    }
                    ?>
                </select>
